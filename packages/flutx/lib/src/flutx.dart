...
class ActionContext<T> {
  Future dispatch(Action type, [dynamic payload = null]) {}

  T get state => null;

  RootState get rootState => null;
}

class Action<T> extends Equatable {
  Future<void> action(ActionContext context) async {
    await context.dispatch(this);
  }

  dynamic get arg => null;
...
class Store<T> {
  Store(
    T initialState, {
    List<Middleware<T>> middlewares = const [],
  });
